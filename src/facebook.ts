...
export function parseMbasicContent(html: string): FbContent {
  ...
  if (hdMatch) result.videos.push(hdMatch[1].replace(/\\\//g, '/'));
  if (playbackHdMatch) result.videos.push(playbackHdMatch[1].replace(/\\\//g, '/'));
  if (sdMatch) result.videos.push(sdMatch[1].replace(/\\\//g, '/'));
  if (playbackMatch) result.videos.push(playbackMatch[1].replace(/\\\//g, '/'));
  if (srcMatch) result.videos.push(srcMatch[1].replace(/\\\//g, '/'));
  ...
  if (jsonLdMatch) {
    try {
      ...
      if (jsonLd.contentUrl) result.videos.push(jsonLd.contentUrl);
      if (jsonLd.embedUrl) result.videos.push(jsonLd.embedUrl);
    } catch { /* ignore */ }
  }
